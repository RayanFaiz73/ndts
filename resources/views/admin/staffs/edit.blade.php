<x-app-layout>
            ...
            <div class="relative flex items-center justify-between h-10">
                ...
                <div
                    ...
                    <div class="relative ml-3">
                        <div>
                            <x-primary-link class="ml-3 text-theme-secondary-100" :href="route('admin.staff.index')">
                                {{ __('All '.$heading.'s')}}
                            </x-primary-link>
                        </div>
                    </div>
            ...
            <div
                ...
                <div class="p-6 text-theme-secondary-100">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form class="w-full" action="{{ route('admin.staff.update',$staff->id) }}" method="POST"
                                ...
                                <div class="flex flex-wrap -mx-3 mb-6">
                                    <div class="w-full lg:w-1/2 px-3 mb-6 lg:mb-3">
                                        ...
                                        <input required="required" name="name" value="{{ old('name',$staff->name) }}"
                                    ...
                                    <div class="w-full lg:w-1/2 px-3 mb-6 lg:mb-3">
                                        ...
                                        <input required="required" name="email" value="{{ old('email',$staff->email) }}"
                                    ...
                                    <div class="w-full lg:w-1/2 px-3 mb-6 lg:mb-3">
                                        ...
                                        <select required name="status" id="status"
                                            ...
                                            <option value="pending" {{$staff->status == 'pending' ? 'selected' : ''}}>{{
                                                __('Pending') }}</option>
                                            <option value="suspended" {{$staff->status == 'suspended' ? 'selected' :
                                                ''}}>{{ __('Suspended') }}</option>
                                            <option value="approved" {{$staff->status == 'approved' ? 'selected' :
                                                ''}}>{{ __('Approved') }}</option>
                                        </select>
                                        @error('status')
                                        <p class="text-theme-danger-500 text-xs italic">{{ $message }}</p>
                                    ...
                                    <div class="w-full lg:w-2/2 px-3 mb-6 lg:mb-3">
                                        ...
                                        <select required name="parent_id" id="hospital_select"
                                            ...
                                            <option value="{{ $hospital->id }}" @if ($hospital->id == $staff->parent_id)
                                                selected @endif>{{ $hospital->data_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('parent_id')
                                        <p class="text-theme-danger-500 text-xs italic">{{ $message }}</p>
    ...
    provinceSelect.addEventListener('change', function() {
        ...
        const filteredHospitals = hospitals.filter(hospital => hospital.parent_id === selectedProvinceId);
        ...
        const staffParentId = staff.parent_id;
        const optionToSelect = hospitalSelect.querySelector(`option[value="${staffParentId}"]`);
        if (optionToSelect) {
